...
function Source.complete(self, args)
  ...
  self.client.request('textDocument/completion', request, function(err, _, response)
      ...
      if self.filetype == "rust" then 
          for i, v in pairs(response.items) do
              if v.label and v.detail and v.kind > 1 and v.kind < 5 and not string.find(v.label, "!") then
                  if string.sub(v.detail, 1, 3) == "fn(" and string.sub(v.label, -5) == "(…)" then
                      response.items[i].label = string.sub(v.label, 1, string.len(v.label) - 5) .. string.sub(v.detail, 3)
                  end
              end
          end
      end
